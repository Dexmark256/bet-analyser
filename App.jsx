...
async function submitFeedback(deviceId, message, rating) {
  // Best-effort WhatsApp ping — fires in parallel, never blocks or fails the submission.
  fetch("/api/feedback", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ message, rating, deviceId }),
  }).catch(() => { /* silent — WhatsApp is a bonus notification, not the source of truth */ });
...
function bestWorstBet(settled) {
  if (!settled.length) return { best: null, worst: null };
  let best = settled[0], worst = settled[0];
  settled.forEach((b) => {
    const p = profitFor(b);
    if (p > profitFor(best)) best = b;
    if (p < profitFor(worst)) worst = b;
  });
  return { best, worst };
}

// ---------- best/worst day ----------
function bestWorstDay(settled) {
  const map = {};
  settled.forEach((b) => { map[b.date] = (map[b.date] || 0) + profitFor(b); });
  const entries = Object.entries(map);
  if (!entries.length) return { best: null, worst: null };
  const best = entries.reduce((a, b) => (b[1] > a[1] ? b : a));
  const worst = entries.reduce((a, b) => (b[1] < a[1] ? b : a));
  return { best: { date: best[0], profit: best[1] }, worst: { date: worst[0], profit: worst[1] } };
}

// ---------- historical average profit per period ----------
...
function groupProfitByPeriod(settled, unit) {
  const map = {};
  settled.forEach((b) => {
    const d = new Date(b.date);
    const key = unit === "week" ? isoWeekKey(d) : unit === "month" ? `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}` : `${d.getFullYear()}`;
    map[key] = (map[key] || 0) + profitFor(b);
  });
...
// ---------- staking / compounding projection ----------
// Deterministic expected-value simulation using the user's own historical
// win rate and average odds on single bets. Not a guarantee — real results
// vary bet to bet. If the underlying edge is negative, this shows the
// bankroll shrinking, not growing, on purpose.
function simulateCompounding({ startBankroll, stakeCap, reinvestPct, winRate, avgOdds, betsPerWeek, weeks }) {
  let bankroll = startBankroll;
  const totalBets = Math.max(1, Math.round(betsPerWeek * weeks));
  for (let i = 0; i < totalBets; i++) {
    const rawStake = bankroll * (reinvestPct / 100);
    const stake = Math.max(0, Math.min(rawStake, stakeCap));
    const ev = stake * ((winRate / 100) * avgOdds - 1);
    bankroll = Math.max(0, bankroll + ev);
  }
  return bankroll;
}
...
function SuggestedBetsPanel({ bets }) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <p style={{ color: "#8A8F87", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.4, margin: "0 0 4px" }}>Suggested bets</p>
      <p style={{ fontSize: 11, color: "#5F655C", margin: "0 0 10px", lineHeight: 1.5 }}>
      ...
      {suggestions.length === 0 ? (
        ...
        suggestions.map((s, i) => (
          <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: "#141A21", border: "1px solid #262E36", borderRadius: 10, padding: "10px 12px", marginBottom: 8 }}>
            <div>
            ...
            <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 20, fontWeight: 700, color: "#E8C766", margin: 0 }}>{s.rate.toFixed(0)}%</p>
          </div>
        ))
      )}
    </div>
...
function ExtremesPanel({ settled }) {
  const { best, worst } = useMemo(() => bestWorstBet(settled), [settled]);
  const { best: bestDay, worst: worstDay } = useMemo(() => bestWorstDay(settled), [settled]);
  if (!best) return null;

  const card = (label, value, sub, color) => (
    <div style={{ flex: 1, minWidth: 140, background: "#141A21", borderRadius: 10, padding: "12px 10px" }}>
      <p style={{ fontSize: 11, color: "#8A8F87", margin: "0 0 4px" }}>{label}</p>
      <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 18, fontWeight: 700, color, margin: 0 }}>{value}</p>
      {sub && <p style={{ fontSize: 11, color: "#5F655C", margin: "2px 0 0" }}>{sub}</p>}
    </div>
  );

  return (
    <div style={{ marginBottom: 20 }}>
      <p style={{ color: "#8A8F87", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.4, margin: "0 0 8px" }}>Highs and lows</p>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
        {card("Highest profit ever", fmtMoney(profitFor(best)), `${best.date} · single bet`, "#6FCF97")}
        {card("Biggest loss", fmtMoney(profitFor(worst)), `${worst.date} · single bet`, "#E0654F")}
      </div>
      {bestDay && (
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {card("Best day", fmtMoney(bestDay.profit), bestDay.date, "#6FCF97")}
          {card("Worst day", fmtMoney(worstDay.profit), worstDay.date, "#E0654F")}
        </div>
      )}
    </div>
  );
...
function AveragesPanel({ settled }) {
  const weekly = useMemo(() => groupProfitByPeriod(settled, "week"), [settled]);
  const monthly = useMemo(() => groupProfitByPeriod(settled, "month"), [settled]);
  const yearly = useMemo(() => groupProfitByPeriod(settled, "year"), [settled]);

  const card = (label, arr) => (
    <div style={{ flex: 1, minWidth: 100, background: "#141A21", borderRadius: 10, padding: "12px 10px" }}>
      <p style={{ fontSize: 11, color: "#8A8F87", margin: "0 0 4px" }}>{label}</p>
      <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 18, fontWeight: 700, color: average(arr) >= 0 ? "#6FCF97" : "#E0654F", margin: 0 }}>
        {average(arr) >= 0 ? "+" : ""}{fmtMoney(average(arr))}
      </p>
      <p style={{ fontSize: 10, color: "#5F655C", margin: "2px 0 0" }}>across {arr.length} {arr.length === 1 ? "period" : "periods"}</p>
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <p style={{ color: "#8A8F87", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.4, margin: "0 0 4px" }}>Average profit achieved</p>
      <p style={{ fontSize: 11, color: "#5F655C", margin: "0 0 10px" }}>Real historical averages — not a projection.</p>
      <div style={{ display: "flex", gap: 8 }}>
        {card("Per week", weekly)}
        {card("Per month", monthly)}
        {card("Per year", yearly)}
      </div>
    </div>
  );
}
...
function LossMinimizationPanel({ settled }) {
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <p style={{ color: "#8A8F87", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.4, margin: "0 0 8px" }}>Minimizing losses</p>

      <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        <div style={{ flex: 1, background: "#141A21", borderRadius: 10, padding: "12px 10px" }}>
          <p style={{ fontSize: 11, color: "#8A8F87", margin: "0 0 4px" }}>Longest losing streak</p>
          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 20, fontWeight: 700, color: "#E0654F", margin: 0 }}>{lossStreak} bets</p>
        </div>
        <div style={{ flex: 1, background: "#141A21", borderRadius: 10, padding: "12px 10px" }}>
          <p style={{ fontSize: 11, color: "#8A8F87", margin: "0 0 4px" }}>Longest winning streak</p>
          <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 20, fontWeight: 700, color: "#6FCF97", margin: 0 }}>{winStreak} bets</p>
        </div>
      </div>

      <div style={{ background: "#141A21", borderRadius: 10, padding: "12px 14px" }}>
...
function CompoundingCalculator({ settled }) {
  ...
  const [reinvestPct, setReinvestPct] = useState("10");
  const stakeCap = 50000;
  ...
  const checkpoints = [
    ...
    label,
    simulateCompounding({
      startBankroll: Number(bankroll) || 0, stakeCap, reinvestPct: Number(reinvestPct) || 0,
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <p style={{ color: "#8A8F87", fontSize: 12, textTransform: "uppercase", letterSpacing: 0.4, margin: "0 0 4px" }}>Compounding calculator</p>
      <p style={{ fontSize: 11, color: "#5F655C", margin: "0 0 10px", lineHeight: 1.5 }}>
        {hasData
          ? `Using your own historical single-bet win rate (${winRate.toFixed(0)}%) and average odds (${avgOdds.toFixed(2)}). Stake is capped at UGX ${stakeCap.toLocaleString()} per bet, whatever percentage you set below.`
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
        <div style={{ flex: 1 }}>
          <label style={label}>Starting bankroll (UGX)</label>
          <input type="number" value={bankroll} onChange={(e) => setBankroll(e.target.value)} style={inputStyle} />
        </div>
        <div style={{ flex: 1 }}>
          <label style={label}>Stake % of bankroll per bet</label>
          <input type="number" value={reinvestPct} onChange={(e) => setReinvestPct(e.target.value)} style={inputStyle} />
        </div>
      </div>
      ...
      <div style={{
        ...
        <p style={{ fontSize: 12, color: edge >= 0 ? "#F1EFE7" : "#E0654F", margin: 0, lineHeight: 1.5 }}>
          {edge >= 0
            ? `At this win rate and odds, each bet has a positive expected value (roughly +${(edge * 100).toFixed(1)}% of stake). Compounding can grow your bankroll over time — but individual bets will still win or lose, this is only the average.`
            : `At this win rate and odds, each bet has a negative expected value (roughly ${(edge * 100).toFixed(1)}% of stake). Compounding would shrink your bankroll on average, not grow it — the numbers below reflect that honestly.`}
        </p>
      </div>

      <div>
        {checkpoints.map(([label, value]) => (
          <div key={label} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #1D242B" }}>
            <span style={{ fontSize: 13, color: "#F1EFE7" }}>{label}</span>
            <span style={{ fontSize: 13, fontWeight: 600, color: "#E8C766" }}>{fmtMoney(value)}</span>
          </div>
        ))}
      </div>

      <p style={{ fontSize: 11, color: "#5F655C", margin: "12px 0 0", lineHeight: 1.5 }}>
        This is a projection from your own average numbers, not a promise — real results vary a lot bet to bet. Never stake more than you can afford to lose, and treat the 50,000 UGX cap as a hard ceiling regardless of what the percentage above suggests.
      </p>
    </div>
  );
}
      ...
      <div style={{ padding: "0 16px" }}>
        ...
        {settled.length === 0 ? (
          ...
          <>
            ...
            <ExtremesPanel settled={settled} />
            <AveragesPanel settled={settled} />
            ...
            <CompoundingCalculator settled={settled} />

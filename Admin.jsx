...
import { LogOut, Users, TrendingUp, AlertTriangle, MessageSquare, Trash2, RefreshCw } from "lucide-react";
...
function Dashboard({ onLogout }) {
  ...
  const load = async () => {
    ...
    try {
      const [d, s, f, e, p] = await Promise.all([
      ...
      setDevices(d.data || []); setStats(s.data || []); setFeedback(f.data || []);
      ...
      {loading ? (
        ...
        <>
          ...
          <Section title={`User feedback (${feedback.length})`}>
            {feedback.length === 0 ? (
              <p style={{ fontSize: 13, color: "#5F655C" }}>No feedback submitted yet.</p>
            ) : (
              feedback.map((f) => (
                <div key={f.id} style={{ background: "#141A21", border: "1px solid #262E36", borderRadius: 10, padding: 12, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <MessageSquare size={13} color="#8A8F87" />
                      {f.rating ? <span style={{ color: "#E8C766", fontSize: 12 }}>{"★".repeat(f.rating)}{"☆".repeat(5 - f.rating)}</span> : null}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ fontSize: 11, color: "#5F655C" }}>{timeAgo(f.created_at)}</span>
                      <button onClick={() => deleteFeedback(f.id)} style={{ background: "none", border: "none", color: "#5F655C", cursor: "pointer" }}><Trash2 size={13} /></button>
                    </div>
                  </div>
                  <p style={{ fontSize: 13, color: "#F1EFE7", margin: 0, lineHeight: 1.5 }}>{f.message}</p>
                </div>
              ))
            )}
          </Section>

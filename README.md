...
ROI — never team names or bet details), page load times, and JS error
messages. Everything else stays local.
...
A floating WhatsApp button sits above the tab bar on every screen.
...
WhatsApp "click to chat" link (`wa.me`). This is the only way users reach
you directly; there's no separate feedback form in the app.
